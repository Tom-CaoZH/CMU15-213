...
### level4 : the target is the same as the level2

#### thinkings:

我们想要达到的目的就是把cookies放入rdi然后跳转到touch2,但是这个时候我们不能在stack里面执行指令，所以我们需要把我们所想要达到的目的分为几个步骤，然后利用farm 里面现有的指令来达到我们想达到的目的

#### implement

##### 1. 经过搜索我发现仅有的方法就是先把cookies pop到rax里面然后把rax move 到rdi，这样目的就可以达到了，这里farm里面给的看似奇怪的坐标其实就是我们可以通过裁剪和组合得到想要的指令

##### 2. 这里其实有一个问题就是我该怎么把cookies以整数的形式输入到内存里面。

###### 这里利用函数getval_280得到pop rax指令

   87 00000000000000ac <getval_280>:

   88   ac:   f3 0f 1e fa             endbr64 

   89   b0:   55                      push   %rbp

   90   b1:   48 89 e5                mov    %rsp,%rbp

   91   b4:   b8 29 58 90 c3          mov    $0xc3905829,%eax

   92   b9:   5d                      pop    %rbp

   **58 90 c3  就是我们想要的，然后通过gdb得到地址为0x00000000004019cc -----> address1**

###### 然后利用函数set_val426得到movq rax rdi（这里说明一下为什么不用set_val237，因为426后面跟的是90是nop指令，对程序没影响，而237后面跟的c7不可取）.

   76 0000000000000093 <setval_426>:

   77   93:   f3 0f 1e fa             endbr64 

   78   97:   55                      push   %rbp

   79   98:   48 89 e5                mov    %rsp,%rbp

   80   9b:   48 89 7d f8             mov    %rdi,-0x8(%rbp)

   81   9f:   48 8b 45 f8             mov    -0x8(%rbp),%rax

   82   a3:   c7 00 48 89 c7 90       movl   $0x90c78948,(%rax)
   ...
   **48 89 c7就是我们想要的，然后通过gdb得到地址为0x00000000004019c5 --------> address2**

#### result

stack里面的顺序： top -------- ......(buffer) ----- > address1 ----> cookies --> address2 -----> touch2_address ---------bottom

**buffer is not the stack**

 1. 这里其实有一点疑惑就是这里pop rbp究竟会pop掉多少，这个我是不清楚的

 2. 这里其实还有一个问题就是我该怎么把cookies以整数的形式输入到内存里面。

 **waring:这里看rtarget的objdump文件，这样就少了调用函数的最后的pop环节（就像上面你纠结的pop多少其实在真正的程序里面没有）**

 **对于第一个问题竟然跟我之前想的一样（xs），就是按小端规则输入**
